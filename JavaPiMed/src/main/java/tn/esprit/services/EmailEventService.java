...
public class EmailEventService {
    ...
    static {
        ...
        BREVO_API_KEY = firstNonBlank(
                getSetting("APIKEY_EVENT", "api.key.event", null),
                getSetting("BREVO_API_KEY", "brevo.api.key", null)
        );
        String configuredSender = getSetting("BREVO_SENDER_EMAIL_EVENT", "brevo.sender.email", null);
        if (isBlank(configuredSender)) {
            configuredSender = getSetting("MEDFLOW_MAIL_USER", "mail.user", null);
        }
        SENDER_EMAIL = configuredSender;
        SENDER_NAME = getSetting("BREVO_SENDER_NAME_EVENT", "brevo.sender.name", "MedFlow");
    }
    ...
    private static String getSetting(String envKey, String propKey, String defaultValue) {
        ...
        String dotEnv = readDotEnvValue(envKey, ".env.local");
        if (dotEnv != null && !dotEnv.isBlank()) return dotEnv;
        dotEnv = readDotEnvValue(envKey, ".env");
        if (dotEnv != null && !dotEnv.isBlank()) return dotEnv;
        return defaultValue;
    }
    ...
    public static void sendEventOnlineEmail(String toEmail, String participantName, Evenement evenement, String roomUrl, boolean update) {
        if (isBlank(toEmail) || evenement == null) return;

        String safeParticipant = isBlank(participantName) ? "Participant" : participantName;
        String title = safe(evenement.getTitre_event()).isBlank() ? "Votre evenement MedFlow" : evenement.getTitre_event();
        String subject = update
                ? "MedFlow - Mise a jour de votre evenement en ligne"
                : "MedFlow - Votre evenement est maintenant en ligne";

        String intro = update
                ? "Des informations de votre evenement en ligne ont ete mises a jour par l'equipe MedFlow."
                : "Votre evenement est desormais disponible en ligne. Vous pouvez rejoindre la salle depuis le lien ci-dessous.";
        String roomPoint = isBlank(roomUrl)
                ? "Le lien de reunion n'est pas encore configure. Consultez MedFlow ou contactez l'organisateur pour le recevoir."
                : "Lien de reunion : <a href='" + escapeHtml(roomUrl) + "' style='color:#0b79a3;font-weight:700;'>" + escapeHtml(roomUrl) + "</a>";

        String html = buildHtml(
                "Bonjour " + escapeHtml(safeParticipant) + ",",
                intro,
                new String[]{
                        "Evenement : <strong>" + escapeHtml(title) + "</strong>",
                        "Dates : <strong>" + escapeHtml(formatEventDateRange(evenement)) + "</strong>",
                        "Lieu / organisation : <strong>" + escapeHtml(safe(evenement.getNom_lieu_event()).isBlank() ? "En ligne" : evenement.getNom_lieu_event()) + "</strong>",
                        roomPoint,
                        "Merci de rejoindre la salle a l'heure prevue ou de contacter l'organisateur en cas d'indisponibilite."
                },
                "#0e8fb0",
                update ? "Mise a jour evenement en ligne" : "Evenement en ligne"
        );
        sendAsync(toEmail, subject, html);
    }

    public static void sendEventOnlineEmailNow(String toEmail, String participantName, Evenement evenement, String roomUrl, boolean update) throws Exception {
        if (isBlank(toEmail) || evenement == null) return;
    ...
    private static void send(String to, String subject, String html) throws Exception {
        if (isBlank(BREVO_API_KEY) || isBlank(SENDER_EMAIL)) {
            System.err.println("[EmailService] Brevo non configure. Definissez APIKEY_EVENT (ou BREVO_API_KEY) et BREVO_SENDER_EMAIL.");
    ...
    private static String formatEventDateRange(Evenement evenement) {
        if (evenement == null) {
            return "-";
        }

        String start = evenement.getDate_debut_event() == null
                ? "-"
                : toLocalDate(evenement.getDate_debut_event()).format(EVENT_DATE_FORMAT);
        String end = evenement.getDate_fin_event() == null
                ? start
                : toLocalDate(evenement.getDate_fin_event()).format(EVENT_DATE_FORMAT);
        return start.equals(end) ? start : start + " au " + end;
    }

    private static String readDotEnvValue(String key, String fileName) {
    ...
    private static java.time.LocalDate toLocalDate(java.util.Date date) {
